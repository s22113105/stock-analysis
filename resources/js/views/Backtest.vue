        ...
        <v-row>
            ...
            <v-col cols="12" md="4">
                <v-card elevation="2">
                    ...
                    <v-card-text>
                        <!-- 股票選擇 -->
                        <v-autocomplete v-model="selectedStock" :items="stockList" item-title="label" item-value="id"
                            label="選擇股票" prepend-icon="mdi-chart-line" density="compact" class="mb-4"
                            :loading="loadingStocks" @update:modelValue="onStockChange"></v-autocomplete>

                        <v-alert v-if="stockLoadError" type="error" density="compact" variant="tonal" class="mb-4">
                            {{ stockLoadError }}
                            <v-btn size="x-small" variant="text" class="ml-2" @click="loadStocks">重試</v-btn>
                        </v-alert>

                        <!-- 策略選擇 -->
                        <v-select v-model="strategy" :items="strategies" item-title="display_name" item-value="name"
                            label="回測策略" prepend-icon="mdi-strategy" density="compact" class="mb-4"
                            @update:modelValue="onStrategyChange"></v-select>

                        <v-alert v-if="strategyLoadError" type="error" density="compact" variant="tonal" class="mb-4">
                            {{ strategyLoadError }}
                            <v-btn size="x-small" variant="text" class="ml-2" @click="loadStrategies">重試</v-btn>
                        </v-alert>

                        <!-- 日期範圍 -->
                        <v-text-field v-model="startDate" label="開始日期" type="date" density="compact"
                            prepend-icon="mdi-calendar-start" class="mb-4"></v-text-field>

                        <v-text-field v-model="endDate" label="結束日期" type="date" density="compact"
                            prepend-icon="mdi-calendar-end" class="mb-4"></v-text-field>

                        <!-- 初始資金 -->
                        <v-text-field v-model.number="initialCapital" label="初始資金" type="number" prefix="NT$"
                        ...
                        <template v-if="strategy === 'sma_crossover'">
                            <v-text-field v-model.number="parameters.short_period" label="短期均線" type="number" suffix="天"
                                density="compact" class="mb-3"></v-text-field>
                            <v-text-field v-model.number="parameters.long_period" label="長期均線" type="number" suffix="天"
                                density="compact" class="mb-3"></v-text-field>
                        </template>

                        <!-- MACD 參數 -->
                        <template v-if="strategy === 'macd'">
                            <v-text-field v-model.number="parameters.fast_period" label="快線期間" type="number"
                                density="compact" class="mb-3"></v-text-field>
                            <v-text-field v-model.number="parameters.slow_period" label="慢線期間" type="number"
                                density="compact" class="mb-3"></v-text-field>
                            <v-text-field v-model.number="parameters.signal_period" label="訊號線期間" type="number"
                                density="compact" class="mb-3"></v-text-field>
                        </template>

                        <!-- RSI 參數 -->
                        <template v-if="strategy === 'rsi'">
                            <v-text-field v-model.number="parameters.period" label="RSI 期間" type="number"
                                density="compact" class="mb-3"></v-text-field>
                            <v-text-field v-model.number="parameters.oversold" label="超賣線" type="number"
                                density="compact" class="mb-3"></v-text-field>
                            <v-text-field v-model.number="parameters.overbought" label="超買線" type="number"
                                density="compact" class="mb-3"></v-text-field>
                        </template>

                        <!-- 布林通道參數 -->
                        <template v-if="strategy === 'bollinger_bands'">
                            <v-text-field v-model.number="parameters.period" label="期間" type="number" suffix="天"
                                density="compact" class="mb-3"></v-text-field>
                            <v-text-field v-model.number="parameters.std_dev" label="標準差倍數" type="number" step="0.1"
                                density="compact" class="mb-3"></v-text-field>
                        </template>

                        <!-- 執行按鈕 -->
                        <v-btn color="primary" size="large" block prepend-icon="mdi-play" @click="runBacktest"
                            :loading="running" :disabled="!selectedStock">
                            執行回測
                        </v-btn>

                        <v-btn color="secondary" size="large" block prepend-icon="mdi-content-save" class="mt-2"
                            @click="saveStrategy" :disabled="!backtestResults">
                            儲存策略
                        </v-btn>
                    </v-card-text>
                </v-card>
            ...
            <v-col cols="12" md="8">
                    ...
                    <v-col cols="6" md="3">
                        <v-card>
                            <v-card-text>
                                ...
                                <div class="text-h5"
                                    :class="backtestResults.total_return >= 0 ? 'text-success' : 'text-error'">
                    ...
                    <v-col cols="6" md="3">
                        <v-card>
                            <v-card-text>
                                ...
                                <div class="text-h5"
                                    :class="backtestResults.annual_return >= 0 ? 'text-success' : 'text-error'">
                ...
                <v-card elevation="2" v-if="backtestResults && tradeHistory.length > 0">
                    <v-card-title class="d-flex justify-space-between align-center">
                        <span>交易明細</span>
                        <v-btn size="small" prepend-icon="mdi-download" @click="exportTrades">
                            匯出 CSV
                        </v-btn>
                    </v-card-title>
                    <v-card-text>
                        <v-data-table :headers="tradeHeaders" :items="tradeHistory" items-per-page="10"
                            class="elevation-0">
                            ...
                            <template v-slot:item.pnl_percent="{ item }">
                                <span v-if="item.pnl_percent !== null"
                                    :class="item.pnl_percent >= 0 ? 'text-success' : 'text-error'">
