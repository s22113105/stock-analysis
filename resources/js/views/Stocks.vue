<template>
    <div class="stocks-page">
        ...
        <v-row class="mb-4">
            <v-col cols="12" md="3">
                <v-text-field v-model="search" label="搜尋股票" prepend-inner-icon="mdi-magnify" variant="outlined"
                    density="compact" clearable hint="輸入股票代碼或名稱"></v-text-field>
            </v-col>
            <v-col cols="12" md="3">
                <v-select v-model="marketFilter" label="市場別" :items="markets" variant="outlined" density="compact"
                    clearable></v-select>
            </v-col>
            <v-col cols="12" md="3">
                <v-select v-model="industryFilter" label="產業別" :items="industries" variant="outlined" density="compact"
                    clearable></v-select>
            </v-col>
            <v-col cols="12" md="3">
                <v-select v-model="changeFilter" label="漲跌篩選" :items="changeOptions" variant="outlined"
            ...
            <v-col cols="12">
                <v-card elevation="2">
                    ...
                    <v-card-text>
                        ...
                        <v-alert v-if="errorMessage" :type="stocks.length > 0 ? 'warning' : 'error'" variant="tonal"
                            class="my-4" closable @click:close="errorMessage = ''">
                            {{ errorMessage }}
                        </v-alert>

                        <!-- 表格 -->
                        <v-data-table :headers="headers" :items="filteredStocks" :search="search" :loading="loading"
                            loading-text="載入資料中..." :no-data-text="loading ? '載入中...' : '暫無股票資料'"
                            ...
                            <template v-slot:item.change="{ item }">
                                <v-chip v-if="item.change !== null" :color="getChangeColor(item.change)" size="small">
                                    <v-icon size="small">{{ item.change >= 0 ? 'mdi-arrow-up' : 'mdi-arrow-down'
                                    }}</v-icon>
                            ...
                            <template v-slot:item.actions="{ item }">
                                <v-btn icon="mdi-eye" size="small" variant="text" @click="viewStockDetail(item)"
                                    title="詳情"></v-btn>
                                <v-btn icon="mdi-calculator-variant" size="small" variant="text"
                                    @click="calculate(item)" title="選擇權分析"></v-btn>
                            </template>
                        </v-data-table>
                    </v-card-text>
                </v-card>
            ...
            <v-card v-if="selectedStock">
                <v-card-title class="d-flex align-center">
                    <div>
                        ...
                        <div class="text-caption text-grey">{{ selectedStock.market }} | {{ selectedStock.industry }}
                        </div>
                    </div>
                    <v-spacer></v-spacer>
                ...
                <v-card-text>
                    ...
                    <div class="d-flex align-center justify-space-between mb-3">
                        ...
                        <v-btn-group density="compact" variant="outlined">
                            <v-btn v-for="p in periods" :key="p.value" :color="chartPeriod === p.value ? 'primary' : ''"
                                @click="changePeriod(p.value)" size="small">{{ p.label }}</v-btn>
                        </v-btn-group>
                    </div>
