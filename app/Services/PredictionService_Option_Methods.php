<template>
    <div class="prediction-page">
            ...
            <v-col cols="12">
                <v-card elevation="2">
                    ...
                    <v-card-text>
                        ...
                        <!-- 預測結果 - 大卡片顯示 -->
                        <v-row v-if="predictionResult" class="mt-4">
                            <v-col cols="12">
                                <v-card
                                    ...
                                    <v-card-text class="pa-8">
                                        ...
                                        <div class="text-h6 mb-4 opacity-90">
                                            {{ predictionResult.target_info.option_code }}
                                            <v-chip
                                        ...
                                        <v-row align="center">
                                            <!-- 左側：當前價格 -->
                                            <v-col cols="12" md="4">
                                                <div class="text-caption opacity-70">今日收盤價</div>
                                            ...
                                            <v-col cols="12" md="1" class="text-center">
                                                <v-icon size="48" class="prediction-arrow">
                                                    {{ getPredictionChange() >= 0 ? 'mdi-arrow-right-thick' : 'mdi-arrow-right-thick' }}
                                                </v-icon>
                                            </v-col>

                                            <!-- 右側：預測價格 -->
                                            <v-col cols="12" md="4">
                                                <div class="text-caption opacity-70">明日預測價格</div>
                                                <div class="text-h3 font-weight-bold">
                                                    ${{ predictedPrice }}
                                                </div>
                                            ...
                                            <v-col cols="12" md="3">
                                                <v-card color="white" class="pa-4">
                                                    <div class="text-h6 text-grey-darken-3 mb-2">
                                                        <v-icon :color="getRecommendationColor()">mdi-lightbulb</v-icon>
                                                        交易建議
                                                    </div>
                                                    <div class="text-h4 font-weight-bold" :style="{ color: getRecommendationColor() }">
                                                        {{ getRecommendation() }}
                                                    </div>
                                        ...
                                        <v-row class="mt-4">
                                            <v-col cols="12">
                                                <div class="text-caption opacity-70 mb-2">
                                                    95% 信賴區間：${{ confidenceLower }} ~ ${{ confidenceUpper }}
                                                </div>
                                                <v-progress-linear
                                                    :model-value="50"
                                                    height="8"
                                                    color="white"
                                                    class="opacity-50"></v-progress-linear>
                                            </v-col>
                                        </v-row>
                                    </v-card-text>
                                </v-card>
                        ...
                        <!-- 預測走勢圖（可選顯示） -->
                        <v-row v-if="predictionResult && showChart">
                            <v-col cols="12">
                        ...
                        <v-row v-if="!predictionResult && !loading">
                            <v-col cols="12">
                                <v-card outlined class="text-center pa-12">
                                    ...
                                    <div class="text-h5 mt-4 text-grey-darken-1">選擇合約並執行預測</div>
                                    <div class="text-body-2 text-grey mt-2">
                                        系統將分析歷史資料，預測明日選擇權收盤價
                                    </div>
                                </v-card>
                            </v-col>
                        </v-row>
                        ...
                        <v-row v-if="loading">
                            <v-col cols="12">
                                <v-card outlined class="text-center pa-12">
                                    ...
                                    <div class="text-h6 mt-4">正在分析中...</div>
                                    <div class="text-caption text-grey mt-2">
                                        使用 {{ getModelName(selectedModel) }} 模型預測明日價格（約需 30-60 秒）
                                    </div>
                                </v-card>
                            </v-col>
                        </v-row>
        ...
        <v-dialog v-model="showParametersDialog" max-width="600">
            <v-card>
                <v-card-title>進階設定</v-card-title>
                <v-card-text>
    ...
    export default {
        ...
        setup() {
            ...
            const models = ref([{
                    text: 'LSTM',
                    value: 'lstm',
                    description: '深度學習 - 準確度高'
                },
                {
                    text: 'ARIMA',
                    value: 'arima',
                    description: '統計模型 - 速度快'
                },
                {
                    text: 'GARCH',
                    value: 'garch',
            ...
            const getRecommendation = () => {
                ...
                if (change >= 2) return '建議買進'
                if (change <= -2) return '建議賣出'
                return '建議觀望'
            }

            const getRecommendationReason = () => {
                const change = parseFloat(getPredictionChange())
                if (change >= 2) return '預期明日上漲超過 2%'
                if (change <= -2) return '預期明日下跌超過 2%'
                return '預期明日價格波動較小'
            }
