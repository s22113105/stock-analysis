...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('options', function (Blueprint $table) {
            ...
            $table->string('contract_size', 20)->default('100')->comment('契約規模');
            $table->enum('exercise_style', ['american', 'european'])->default('american')->comment('履約方式');
            $table->boolean('is_active')->default(true)->comment('是否啟用');
            $table->json('meta_data')->nullable()->comment('其他資料');
            $table->timestamps();
            
            $table->index(['stock_id', 'expiry_date']);
